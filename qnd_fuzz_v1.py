        ...
        for axis_name, axis_q in axes[:4]:  # First 4 axes
            for _ in range(n_per_config):
                s = FuzzSample(
                    ...
                    person="3rd",
                    tense="present" if timing == "during" else ("past" if timing == "after" else "future"),
                    voice="active",
                    certainty="definite",
                    emotional="neutral",
                    stakes="serious",
...
def analyze_fuzz_results(results: Dict, samples: List[FuzzSample]) -> Dict:
    ...
    analysis["summary"]["parse_rate"] = parsed_count / total_count if total_count > 0 else 0
    ...
    analysis["by_dimension"]["timing"] = {k: compute_stats(v) for k, v in by_timing.items()}
    analysis["by_dimension"]["abstraction"] = {k: compute_stats(v) for k, v in by_abstraction.items()}
    analysis["by_dimension"]["language"] = {k: compute_stats(v) for k, v in by_language.items()}
    analysis["by_dimension"]["emotional"] = {k: compute_stats(v) for k, v in by_emotional.items()}
    ...
    for pair_key, orders in order_pairs.items():
        ...
        if ab_stats["n"] > 0 and ba_stats["n"] > 0 and ab_stats["mean"] is not None and ba_stats["mean"] is not None:
            diff = abs(ab_stats["mean"] - ba_stats["mean"])
            # Simple t-test approximation
            pooled_se = math.sqrt((ab_stats["variance"] / ab_stats["n"]) + (ba_stats["variance"] / ba_stats["n"])) if ab_stats["variance"] and ba_stats["variance"] else 1
    ...
    if all(k in timing_stats for k in ["before", "during", "after"]):
        ...
        if all(s["mean"] is not None for s in [before, during, after]):
            ...
            analysis["timing_effects"].append({
                ...
                "interference_ratio": interference / expected_classical if expected_classical != 0 else 0
            })
    ...
    if all(v is not None for v in abstraction_values):
        ...
        increasing = abstraction_values[0] <= abstraction_values[1] <= abstraction_values[2]
        decreasing = abstraction_values[0] >= abstraction_values[1] >= abstraction_values[2]
    ...
    if len(lang_means) > 1:
        ...
        variance_across_langs = sum((m - mean_of_means) ** 2 for m in lang_means.values()) / len(lang_means)
    ...
    emotional_means = {k: v["mean"] for k, v in emotional_stats.items() if v["mean"] is not None}
    ...
    if all_axis_means:
        ...
        global_std = math.sqrt(sum((m - global_mean) ** 2 for m in all_axis_means) / len(all_axis_means)) if len(all_axis_means) > 1 else 1
        
        for axis, stats in axis_stats.items():
            if stats["mean"] is not None:
                z_score = (stats["mean"] - global_mean) / global_std if global_std > 0 else 0
                if abs(z_score) > 2:
...
def print_fuzz_analysis(analysis: Dict, output_dir: Path):
    ...
    for t in analysis["timing_effects"]:
        ...
        print(f"  Interference: {t['interference']:+.3f} ({100*t['interference_ratio']:+.1f}%)")
    ...
    for l in analysis["language_effects"]:
        ...
        print(f"  Invariant: {'YES ✓' if l['invariant'] else 'NO - language affects judgment'}")
    ...
    for e in analysis["emotional_effects"]:
        ...
        print(f"  {e['emotion']}: {e['neutral_mean']:.3f} → {e['primed_mean']:.3f} "
    ...
    axis_stats = analysis["by_dimension"]["axis"]
    for axis, stats in sorted(axis_stats.items(), key=lambda x: -(x[1].get("mean") or 0)):
        if stats["mean"] is not None:
            print(f"  {axis:<12}: mean={stats['mean']:.3f}, std={stats['std']:.3f}, n={stats['n']}")
...
def main():
    ...
    parser.add_argument("--n-samples", type=int, default=500, help="Number of random samples")
    parser.add_argument("--structured", action="store_true", help="Use structured fuzzing")
    parser.add_argument("--n-per-config", type=int, default=10, help="Samples per structured config")
    ...
    if args.mode == "submit":
        ...
        # Pre-registration
        prereg = hashlib.sha256(json.dumps({
        ...
        with open(output_dir / f"fuzz_batch_{ts}.json", 'w') as f:
            json.dump({"batch_id": batch.id, "prereg": prereg, "n_samples": len(samples)}, f)
