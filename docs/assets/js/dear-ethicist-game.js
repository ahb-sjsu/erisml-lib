...
(function () {
    ...
    root.innerHTML =
      '<div class="de-start">' +
        ...
        '<div class="de-start-body">' +
          ...
          '<p>You are the advice columnist. Read letters from people facing everyday moral dilemmas, then classify each party\'s <strong>Hohfeldian normative position</strong>:</p>' +
          '<div class="de-positions-legend">' +
            '<span class="de-pos de-pos-O">O <small>Obligation</small></span>' +
            '<span class="de-pos de-pos-C">C <small>Claim</small></span>' +
            '<span class="de-pos de-pos-L">L <small>Liberty</small></span>' +
            '<span class="de-pos de-pos-N">N <small>No-claim</small></span>' +
          '</div>' +
          '<p class="de-hint">The game measures the D<sub>4</sub> correlative symmetry of your moral reasoning: O\u2194C and L\u2194N. Your <strong>Bond Index</strong> tracks structural consistency.</p>' +
          '<div class="de-start-stats">' +
            '<div class="de-ss"><span class="de-ss-num">68</span><span class="de-ss-label">Letters</span></div>' +
  ...
  function renderPlaying() {
    ...
    var partyRows = letter.parties.map(function (p, i) {
      return '<div class="de-party-row" data-party-idx="' + i + '">' +
        '<span class="de-party-name">' + escHtml(p.name) + ' <small class="de-party-role">(' + escHtml(p.role.replace(/_/g, ' ')) + ')</small></span>' +
        '<div class="de-pos-btns">' +
          ['O', 'C', 'L', 'N'].map(function (pos) {
            return '<button class="de-pos-btn" data-pos="' + pos + '">' + pos + '</button>';
          }).join('') +
        '</div>' +
      '</div>';
    }).join('');
    ...
    root.innerHTML =
      '<div class="de-playing">' +
        ...
        '<div class="de-classify">' +
          '<h4>Classify each party\'s normative position:</h4>' +
          partyRows +
        '</div>' +
        '<button class="de-btn de-btn-primary de-publish-btn" id="de-publish" disabled>Publish Your Verdict</button>' +
      '</div>';

    // Wire up position buttons
    root.querySelectorAll('.de-party-row').forEach(function (row) {
      var idx = parseInt(row.dataset.partyIdx);
      row.querySelectorAll('.de-pos-btn').forEach(function (btn) {
        btn.addEventListener('click', function () {
          row.querySelectorAll('.de-pos-btn').forEach(function (b) { b.classList.remove('active'); });
          btn.classList.add('active');
          state.verdicts[idx] = btn.dataset.pos;
          // Enable publish if all parties classified
  ...
  function renderReaction() {
      ...
      if (vd.correlative_holds) {
        symmetryHtml =
          '<div class="de-symmetry de-sym-good">' +
            '<strong>Correlative symmetry holds:</strong> ' + a + ' \u2194 ' + b +
            ' is a valid Hohfeldian correlative pair.' +
          '</div>';
      } else {
        symmetryHtml =
          '<div class="de-symmetry de-sym-warn">' +
            '<strong>Symmetry violation:</strong> ' + a + ' \u2194 ' + b +
    ...
    var verdictSummary = vd.verdicts.map(function (v) {
      return '<span class="de-verdict-chip"><strong>' + escHtml(v.party_name) + ':</strong> ' +
        '<span class="de-pos-tag de-pos-' + v.state + '">' + v.state + '</span></span>';
    }).join('');
  ...
  function renderSummary() {
    ...
    if (bi === 0) {
      ...
      gradeDesc = 'Perfect correlative symmetry. Your moral reasoning preserves the full D\u2084 structure.';
    } else if (bi < 0.2) {
      grade = 'Strong'; gradeClass = 'de-grade-good';
      gradeDesc = 'High structural consistency. Minor deviations from correlative symmetry.';
    } else if (bi < 0.5) {
      grade = 'Moderate'; gradeClass = 'de-grade-moderate';
      gradeDesc = 'Some correlative violations detected. Common patterns like in-group bias or omission bias may be present.';
    } else {
      grade = 'Divergent'; gradeClass = 'de-grade-high';
      gradeDesc = 'Significant correlative asymmetry. Your reasoning may reflect systematic biases that the D\u2084 group detects.';
    }
    ...
    var breakdownRows = state.sessionHistory.map(function (sv, i) {
      ...
      return '<tr>' +
        ...
        '<td>' + sv.verdicts.map(function (v) { return v.state; }).join(', ') + '</td>' +
