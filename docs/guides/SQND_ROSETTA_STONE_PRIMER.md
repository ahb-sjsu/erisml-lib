...
> **measured** structure. The full 8-element D₄ described below is the **posited** ambient
> machinery: it is implemented in the library so the quarter-turn hypothesis stays testable,
> but quarter-turn operations (r, r³, sr, sr³) have not been demonstrated as normative
> operations. Read "D₄" below as "the posited machinery", not "the confirmed result."
...
Note that this example needs the quarter-turn `r` — the posited generator. The two *measured*
operations (`s` and `r²`) commute, so within the measured V₄ subgroup order never matters.
Non-abelian behavior is a prediction of the posited D₄ extension, not an established property.

**Why this matters for AI safety:** If presenting information in different orders produces different moral judgments, the posited D₄ structure predicts *when* that should happen and when it shouldn't. Not all order-dependence is a bug — some is expected at boundaries between competing moral considerations. Finding it would be evidence *for* the quarter-turn sector; on current data it has not been found.
...
| **D₄** | Specific 8-element symmetry group (posited; the measured subgroup is V₄) | 8-node state machine |
...
**Physics framing:** "Measure discrete D₄ gate activation via linguistic triggers" (this protocol probes the *posited* quarter-turn sector)
...
The Hohfeldian gauge structure (V₄ measured, D₄ posited) integrates with the [DEME 2.0 moral landscape framework](docs/DEME_2.0_D4_Integration.md):
...
5. **Rigor:** "We tested it and it seemed fine" is weak. "Correlative (V₄) symmetry verified at 100% (N=400, p < 10⁻⁸)" is strong.
...
| **D₄** | Dihedral group of order 8: symmetries of a square (posited ambient group) |
| **V₄** | Klein four-group {e, s, r², sr²}: the measured subgroup generated by the two demonstrated operations |
...
Note the measured subgroup V₄ = {e, r², s, sr²} is closed under this table (any product of
those four elements lands back in the set) — that closure is exactly what the regression test
`test_hohfeldian_operations_generate_v4` asserts.
...
A: Because Hohfeld's 8 fundamental jural relations map onto D₄'s 8 elements, and D₄ is the full symmetry group of the square the four positions sit on — the natural *ambient* group for this classification scheme. But be precise about status: the correlative and negation operations by themselves are commuting involutions, so they only generate the Klein four-subgroup V₄. The measured group is V₄; D₄ is posited until quarter-turn operations are demonstrated empirically.

**Q: What's the U(1) part?**  
A: U(1) captures the continuous "confidence" or "salience" dimension. The discrete group (V₄ measured, D₄ posited) handles the state, U(1) handles how strongly that state is held. Think of it as: the discrete group picks which bucket, U(1) says how confident we are about the bucket.
