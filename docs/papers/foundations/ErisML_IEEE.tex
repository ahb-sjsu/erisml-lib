...
\item \textbf{Formal semantics} grounded in the Geometric Ethics framework---moral manifold, $V_4$ gauge group ($D_4$ posited), Noether conservation
...
\textbf{Deontic Gauge Group}: The demonstrated symmetries of Hohfeldian normative positions (Obligation, Claim, Liberty, No-claim) are correlative symmetry (perspective swap, $s$) and negation ($r^2$). These are commuting involutions, so the group they generate is the Klein four-group $V_4 = \{e, s, r^2, sr^2\}$ (abelian, order 4)---the \emph{measured} structure. The full dihedral group $D_4$ (order 8) is \emph{posited}: its quarter-turn elements would make the order of perspective-shifting and quarter-turn operations matter ($rs \neq sr$), a prediction that has not yet been demonstrated in human moral reasoning.
...
The \textbf{Bond Index} ($\mathrm{Bd}$) measures representational coherence: $\mathrm{Bd} = 0$ indicates perfect correlative symmetry; the empirical baseline from the Dear Abby corpus (20,030 letters, 1985--2017) is $\mathrm{Bd} = 0.155$. The Hohfeldian gauge structure is exercised through Wilson observable computations; on current data these confirm only the abelian $V_4$ subgroup, and they would detect non-abelian structure if quarter-turn transformations were ever observed.
...
\textbf{Thiele (2026)}~\cite{thiele2026}: A preliminary, unpublished analysis extending BIP testing to \emph{all nine} moral dimensions using 512 labeled moral vignettes and cross-lingual transfer across six typologically diverse languages (English, Spanish, French, Arabic, Chinese, Japanese), the cross-lingual transfer being carried by the multilingual (LaBSE) encoder's language-agnostic representation. This is an unpublished single-author probe study and does not constitute peer-reviewed independent verification.
...
agents with divergent goals. The mathematical foundations from Geometric Ethics~\cite{bond2026ge}---moral manifold, $V_4$ gauge group ($D_4$ posited), Noether conservation, No Escape containment---provide the theoretical grounding; the DEME~2.0 architecture provides the engineering substrate; and the Bond Invariance Principle provides the falsifiability criterion.
