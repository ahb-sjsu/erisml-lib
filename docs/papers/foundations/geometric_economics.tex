...
These are the nine dimensions of the moral manifold identified in Geometric Ethics \cite{bond2026geometric}, empirically validated across 20,030 texts and 11 languages, and examined across 6 additional languages in a preliminary, unpublished analysis \cite{thiele2026} (an unpublished single-author probe study that does not constitute peer-reviewed independent verification; the cross-lingual transfer being carried by the multilingual LaBSE encoder's language-agnostic representation).  The claim is not that these dimensions are unique to ethics.  The claim is that they are the \emph{same dimensions on which economic decisions are made}---because economic decisions are a subset of moral decisions.
