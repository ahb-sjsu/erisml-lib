...
Gauge group & $V_4$ measured, $D_4$ posited (tetrad) & $V_4 \rtimes V_4$ measured (full octad) \\
...
In Geometric Ethics, the measured gauge group of the first four Hohfeldian positions---right, duty, liberty, no-right---is the Klein four-group $V_4$, generated by the correlative and opposite relations; the full dihedral $D_4$ is posited as an ambient group but is licensed only if quarter-turn operations are independently demonstrated as normative operations \cite{bond2026geometric}.  Law requires the \emph{full} Hohfeldian octad:
...
The measured symmetry group of the full Hohfeldian octad is the \emph{semi-direct product}:
\begin{equation}
G_{\Ho} = V_4 \rtimes_\varphi V_4
\label{eq:semidirect}
\end{equation}
where the first factor $V_4$ acts on first-order relations (right--duty--liberty--no-right), the second factor $V_4$ acts on second-order relations (power--liability--immunity--disability), and the homomorphism $\varphi: V_4 \to \operatorname{Aut}(V_4)$ encodes how second-order operations act on first-order positions.  The posited ambient group is $D_4 \rtimes_\varphi D_4$, licensed only if quarter-turn operations on each square are independently demonstrated as legal operations.
\end{theorem}

\begin{proof}[Proof sketch]
Each Hohfeldian square carries $V_4$ symmetry by the same argument as Theorem 11.4 of \cite{bond2026geometric}: the four positions form two correlative pairs connected by jural opposition (negation) and correlative swap (agent-swap).  These are commuting involutions, and two commuting involutions generate the Klein four-group $V_4$---their composition is the $180^\circ$ rotation of the square, never a quarter-turn---so the measured group of each square is $V_4$, with $D_4$ available only as a posited extension.

However, the two squares are \emph{not} independent.  Second-order positions actively constrain and transform first-order positions: exercising a \emph{power} creates or destroys rights and duties; holding an \emph{immunity} prevents another party's power from altering one's first-order position.  Formally: a second-order gauge transformation $g_2 \in V_4$ (e.g., relabeling power $\leftrightarrow$ liability) induces a corresponding transformation on the first-order square (e.g., the right that the power would create is relabeled as the duty that the liability would impose).  This is precisely the structure of a semi-direct product: the second factor acts on the first via the homomorphism $\varphi$.

The group $G_{\Ho} = V_4 \rtimes_\varphi V_4$ has order at most 16 (the same as the direct product; the posited $D_4 \rtimes_\varphi D_4$ would have order at most 64), but its multiplication rule encodes the coupling:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Non-commutativity of normative frameworks (16,798 commutator measurements).}  The order in which normative frameworks are applied changes the outcome ($[U, D] \neq 0$).  This confirms that normative evaluation is order-dependent; note that $U$ and $D$ are framework contraction operators, not elements of the deontic gauge group (which is the abelian $V_4$), so this is an evaluation-order effect rather than evidence of a non-abelian gauge group.  It is directly relevant to law, where the order of applying legal tests (e.g., standing before merits, jurisdiction before substance) produces different analytical paths.
...
\subsection{Preliminary External Analysis (Thiele, 2026)}

The framework's cross-lingual predictions have been examined in a preliminary, unpublished analysis by Thiele \cite{thiele2026}, who reported all nine normative dimensions as linearly decodable from LaBSE embeddings (mean $F_1 = 0.83$) and found cross-lingual transfer across six languages ($F_1$ 0.71--0.82); this cross-lingual transfer is carried by the multilingual (LaBSE) encoder's language-agnostic representation.  Most significantly, the moral-judgement subspace and the language-identity subspace were found to be nearly orthogonal (shared variance $< 3\%$).  This is an unpublished single-author probe study and does not constitute peer-reviewed independent verification.
...
\begin{itemize}
    \item \textbf{The Hohfeldian structure.}  The measured $V_4$ gauge group was derived from Hohfeld's \emph{Fundamental Legal Conceptions} \cite{hohfeld1913}---a legal taxonomy developed for legal reasoning and validated by a century of jurisprudential practice.  The extension to $V_4 \rtimes V_4$ for the full octad (Theorem~\ref{thm:gauge_group}) incorporates the second-order relations (power, immunity, liability, disability) that govern legal \emph{change}---legislation, adjudication, delegation---and that are essential for modeling the judicial complex.
...
Preliminary external analysis (6 languages) & Orthogonality of legal and linguistic structure & Consistent \\
...
Hohfeldian $V_4$ gauge group ($D_4$ posited) & Legal symmetry structure & Measured \\
...
\begin{remark}[Status]
This calibration and validation pipeline has not yet been executed on legal data.  The analogous pipeline for moral dimensions has been validated on 20,030 texts and examined in a preliminary, unpublished analysis \cite{thiele2026}.  Extending it to legal corpora is the immediate empirical priority (Section~\ref{sec:conclusion}).
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{$D_4$ gauge machinery} (implemented so the posited $D_4$ hypothesis remains testable; the measured group is $V_4$) extends to operations on the full Hohfeldian octad.
...
\begin{enumerate}
    ...
    \item \textbf{The Octahedral Gauge Group:} Extension of the measured $V_4$ gauge group to $V_4 \rtimes V_4$ for the full Hohfeldian octad (inside a posited $D_4 \rtimes D_4$), with Wilson loop detection of judicial inconsistency.
...
\bibitem{thiele2026}
L.~Thiele, ``Testing MoralVector Decodability and Cross-Lingual Transfer in LaBSE Embeddings,'' undergraduate research report (unpublished), UCLA, 2026.
