...
\frac{\mathbb{E}_i \bigl[\, \text{mean}_{j \in N_k(i,\, \neg g_i)} \;
     ...
     {\mathbb{E}_i \bigl[\, \text{mean}_{j \in N_k(i,\, g_i)} \;
                            \lvert f(i) - f(j) \rvert\, \bigr]},
\end{equation}
where $N_k(i, c)$ is the set of the $k$ nearest neighbours
$j \neq i$ to individual $i$ in $(V, d_M)$ with $g_j = c$ (writing
$\neg g_i = 1 - g_i$), and $\mathbb{E}_i$ is the empirical mean over
individuals.
\end{definition}
...
(false-positive rate 56.5\,\% at the nominal 5\,\% level), confirming
