...
The Epistemic Invariance Principle is, at its core, a statement about group actions. The ``meaning-preserving transformations'' of EIP form a group; EIP demands that the judgment function be invariant under this group's action on the input space. The Bond Invariance Principle further identifies a deontic symmetry group ($V_4 \times U(1)_\calH$, measured; the full dihedral $D_4 \times U(1)_\calH$ is posited) governing the gauge structure of normative evaluation.
...
The deontic symmetry group is generated by the correlative and negation operations on the four Hohfeldian jural relations---rights, duties, liberties, and no-rights. These two operations are commuting involutions, so the measured group is the Klein four-group $V_4$; the full dihedral group $D_4$ (the symmetry group of the square) is posited as the ambient group. The measured gauge group of geometric ethics is $V_4 \times U(1)_\calH$, where $U(1)_\calH$ is the harm phase group governing conservation.
\end{definition}

\begin{theorem}[Deontic Gauge Group Theorem, informal]
The symmetry group of the deontic structure of bilateral legal/moral relations generated by the Hohfeldian correlative and negation operations is isomorphic to the Klein four-group $V_4$; the full dihedral $D_4$ is posited, not established.
\end{theorem}
