...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Proves that the optimal equilibrium configuration has a specific symmetry structure ($V_4 \times U(1)_\harm$ as measured, sitting inside a posited $D_4 \times U(1)_\harm$) derived from the four fundamental legal relations identified by Hohfeld (1913)---rights, duties, liberties, and no-rights---and their algebraic transformations under change of perspective.
...
\subsection{The Deontic Gauge Group: \texorpdfstring{$V_4$}{V4} Measured, \texorpdfstring{$D_4$}{D4} Posited}
...
\begin{theorem}[Deontic Gauge Group]
\label{thm:d4}
The symmetry group generated by the two Hohfeldian relations, acting on the jural square with bounded harm, is:
\begin{equation}
G = V_4 \times U(1)_\harm
\label{eq:gauge_group}
\end{equation}
where $V_4 = \{e, s, r^2, sr^2\}$ is the Klein four-group (abelian, order 4) generated by the correlative and opposite operations, and $U(1)_\harm$ is the continuous circle group governing harm magnitude. The maximal symmetry group consistent with the square's structure is the dihedral $D_4 \times U(1)_\harm$ (order 8 in the discrete factor); the dihedral extension is \emph{posited}, licensed only if quarter-turn operations ($r, r^3, sr, sr^3$) are independently demonstrated as normative operations.
\end{theorem}

\begin{proof}[Proof sketch]
Label the four Hohfeldian positions as vertices of a square: Right (R), Duty (D), Liberty (L), No-Right (N). The correlative relation is an involution $s$: $R \leftrightarrow D$, $L \leftrightarrow N$. The opposite relation is an involution $r^2$: $R \leftrightarrow N$, $D \leftrightarrow L$. These two involutions \emph{commute}, and their composition $sr^2$ ($R \leftrightarrow L$, $D \leftrightarrow N$) is the third non-identity element; no composition of the two yields a quarter-turn. The group they generate is therefore $\{e, s, r^2, sr^2\} \cong V_4$, the Klein four-group---\emph{not} the full dihedral group $D_4$. (An earlier version of this result claimed $D_4$; that was an overclaim, corrected here. $D_4 = \langle r, s \rangle$ requires the $90^\circ$ rotation $r$ as an independent generator, and $V_4$ sits inside it as an index-2 subgroup.)

The harm magnitude transforms under a continuous phase symmetry $U(1)_\harm$: multiplying all harm values by $e^{i\theta}$ (in the complexified theory) preserves the relative structure. The measured gauge group is the direct product $V_4 \times U(1)_\harm$.

The automorphism group of the square---the ceiling any deontic gauge group must respect---is $D_4$; together with the requirement that harm be bounded below this bounds the discrete factor above by $D_4$ (see \cite{bond2026geometric}, Theorem 11.3 and \cite{bond2026noether}, \S3.2). Whether the quarter-turn elements act as genuine normative operations is an open empirical question.
\end{proof}

The $V_4$ symmetry represents the structural equilibrium where the obligation vectors of interacting agents are perfectly covariant. In this state, the behavioral friction on the network approaches its global minimum.
...
A caution on interpretation---and a correction of our own earlier reading. $U$ and $D$ are \emph{framework contraction operators} acting at the evaluation layer; they are not elements of the deontic gauge group of Theorem~\ref{thm:d4}. The two measured Hohfeldian operations (correlative and opposite) are commuting involutions generating the \emph{abelian} Klein four-group $V_4$, so the commutator measurements cannot be read as evidence for a non-abelian gauge group over the jural square, and they do not ``rule out abelian gauge groups''---an inference we previously drew and here retract. What the 16{,}798 measurements do establish is an operator-ordering effect in framework application: the order in which contraction operators collapse the moral tensor changes the outcome. A non-abelian \emph{deontic} gauge group ($D_4$) would require the quarter-turn operations to be demonstrated empirically as normative operations, which has not been done.
...
\begin{enumerate}
    ...
    \item \textbf{Deontic gauge symmetry:} Implementation of the full $D_4$ group operations on Hohfeldian positions (\texttt{hohfeld.py}), with a test suite verifying the group axioms and verifying that the two Hohfeldian operations generate exactly the Klein four-subgroup $V_4$ (the dihedral machinery is implemented so the $D_4$ hypothesis stays testable), and interactive demonstration (\texttt{hohfeld\_d4\_demo.py}).
