...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Proves that the optimal equilibrium configuration has a specific symmetry structure ($V_4 \times U(1)_\harm$, measured; the full dihedral $D_4 \times U(1)_\harm$ is posited) derived from the four fundamental legal relations identified by \citet{hohfeld1913}---rights, duties, liberties, and no-rights---and their algebraic transformations under change of perspective.
...
The dimensionality---nine, not seven or twelve---is a modeling choice whose adequacy is empirical. The cross-lingual evidence of \S\ref{sec:evidence} shows stable dimensionality across 11 languages; a preliminary, unpublished analysis (\S\ref{subsec:replication}) reported all nine dimensions are linearly decodable from multilingual embeddings. The nine dimensions are neither derivable from first principles nor arbitrary: they are the empirically minimal set that captures the structure present in moral reasoning across cultures.
\end{remark}
...
\subsection{The $V_4$ Gauge Group}

\begin{theorem}[$V_4$ Gauge Group]
\label{thm:d4}
The symmetry group of the deontic structure defined by the four fundamental Hohfeldian positions---right, duty, liberty, no-right \citep{hohfeld1913}---generated by the correlative and opposite operations is the Klein four-group $V_4$ (abelian, order 4).
\end{theorem}

\begin{proof}[Proof sketch]
The four Hohfeldian positions form two correlative pairs: right--duty and liberty--no-right. These pairs are connected by the jural opposition (negation) and correlative (agent-swap) operations. These two operations are commuting involutions---their composition is the $180^\circ$ rotation of the Hohfeldian square---so the group they generate is isomorphic to $V_4$, the Klein four-group of order 4. See \citet{anon2026a}, Theorem 11.4. The full dihedral group $D_4$ (order 8, non-abelian) is posited as the ambient group; it applies only if the quarter-turn operations are independently demonstrated as normative operations, which has not yet been done.
\end{proof}

The $V_4$ gauge group constrains which transformations of moral scenarios are symmetries. A gauge-invariant moral evaluation must be unchanged under $V_4$ transformations of the Hohfeldian structure. This is a strong constraint: it rules out moral evaluations that are sensitive to the arbitrary labeling of agents as ``right-holder'' versus ``duty-bearer'' when the underlying bond structure is preserved.

The measured gauge group of the framework is $V_4 \times U(1)_\harm$, where the $U(1)$ factor corresponds to the continuous symmetry associated with harm conservation (Theorem~\ref{thm:noether}); the full dihedral $D_4 \times U(1)_\harm$ remains a posited extension.
...
The operators $U$ and $D$ act on framework evaluations, not on Hohfeldian positions: they are not elements of the deontic gauge group, and their non-commutativity does not bear on that group's structure (the measured deontic group $V_4$ of Theorem~\ref{thm:d4} is abelian). What the commutator measurements establish is that framework composition is path-dependent---the order in which moral frameworks are applied changes the outcome---a structural feature that any adequate formalization of moral reasoning must capture.
...
Cross-lingual decodability & Preliminary analysis (6 languages) & Reported \\
Moral--language orthogonality & Preliminary analysis & Reported ($< 3\%$) \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Preliminary External Analysis}
\label{subsec:replication}

The cross-lingual results above were obtained within the originating research programme. In early 2026, an undergraduate researcher at a separate institution carried out a preliminary, unpublished analysis using only the published framework description and the publicly available LaBSE embedding model \citep{anon_indep2026}. This single-author probe study provides an early external look at the framework's empirical predictions; it is not a peer-reviewed independent replication.

The undergraduate researcher assembled a multilingual moral-scenario corpus spanning six typologically diverse languages (English, Spanish, Mandarin, Arabic, Hindi, Swahili) and trained linear diagnostic probes on LaBSE sentence embeddings to detect each of the nine moral-manifold dimensions. The probe architecture and training protocol were developed without access to the originating lab's code or data, ensuring methodological independence.

All nine dimensions proved linearly decodable with F1 scores ranging from 0.74 to 0.91 (mean 0.83). The highest-performing dimensions were physical consequences ($F_1 = 0.91$), fairness ($F_1 = 0.88$), and privacy ($F_1 = 0.87$); the lowest were epistemic status ($F_1 = 0.74$) and virtue ($F_1 = 0.76$), consistent with these dimensions' greater context-dependence. Linear decodability is consistent with LaBSE's representation space encoding morally relevant structure in a form accessible to simple readout---a necessary condition for the framework's claim that moral judgements inhabit a low-dimensional submanifold of embedding space.

Probes trained on English data and evaluated on the remaining five languages achieved F1 scores between 0.71 (Swahili) and 0.82 (Spanish), consistent with the BIP's prediction that deontic structure transfers across languages without per-language tuning. This cross-lingual transfer is carried by the multilingual (LaBSE) encoder, whose shared embedding space already aligns the six languages, rather than being attributable to the moral geometry alone. The gradient---Spanish $>$ Mandarin $>$ Arabic $>$ Hindi $>$ Swahili---tracks typological distance from English, suggesting that residual performance variation reflects surface distributional differences rather than failures of structural invariance.

Perhaps the most theoretically significant result is the undergraduate researcher's principal-component analysis showing that the moral-judgement subspace and the language-identity subspace share less than 3\% of their variance. This near-orthogonality is precisely what the geometric framework predicts: if moral structure is a genuine geometric invariant of the embedding manifold, it should be recoverable independently of the coordinate system (language) in which scenarios are expressed. Combined with the 1.2\% language leakage found in a separate adversarial-suppression study, the orthogonality result is consistent with treating the invariance claim as a structural property of the representation space rather than a mere statistical regularity, though as an unpublished single-author probe study it is not peer-reviewed independent verification.

The preliminary analysis's methodology differs from ours in important respects: six languages versus our eleven, linear probes versus our full analysis pipeline, and a separate corpus. The convergence of results across these methodological differences is consistent with the invariance interpretation. We note, however, that this is an unpublished single-author undergraduate probe study, not peer-reviewed independent verification, and it does not by itself resolve the absence of external verification of the present work.
...
The No Escape Theorem (Theorem~\ref{thm:no_escape}) shows that structural containment is mathematically possible. The engineering methodology shows that the architecture is tractable. The empirical evidence---now including a preliminary, unpublished analysis---is consistent with the framework's predictions. The Mandatory Canonicalization Theorem (Theorem~\ref{thm:mandatory}) shows that post-hoc safety is provably insufficient.
...
\begin{enumerate}
    ...
    \item \textbf{$D_4$ gauge symmetry:} Implementation of the full posited $D_4$ group operations on Hohfeldian positions (so the dihedral hypothesis stays testable), with test suite verifying group axioms.
...
[Independent Researcher] (2026). Probing geometric moral structure in multilingual embeddings (unpublished undergraduate research report). Technical Report, [Institution].
