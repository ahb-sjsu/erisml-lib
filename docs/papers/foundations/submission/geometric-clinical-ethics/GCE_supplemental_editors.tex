...
We provide the missing mathematics by porting the Geometric Ethics framework \cite{bond2026geometric}---validated across 20,030 texts and 11 languages, and examined in a preliminary, unpublished analysis \cite{thiele2026}---into clinical medicine.  The clinician computes $f(n) = g(n) + h(n)$, where $g(n)$ is accumulated clinical evidence (evidence-based medicine) and $h(n)$ is the moral-heuristic estimate (clinical wisdom).  The minimum-cost path on the full manifold is the \emph{clinical geodesic}.
