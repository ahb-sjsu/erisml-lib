...
Gauge group & $V_4$ measured, $D_4$ posited (tetrad) & $V_4 \rtimes V_4$ measured, $D_4 \rtimes D_4$ posited (full octad) \\
...
In Geometric Ethics, the measured gauge group of the first four Hohfeldian positions---right, duty, liberty, no-right---is the Klein four-group $V_4$, generated by the correlative and opposite operations; the full dihedral $D_4$ is posited as the ambient group \cite{anon2026a}.  Law requires the \emph{full} Hohfeldian octad:
...
The symmetry group of the full Hohfeldian octad generated by the correlative and opposite operations is the \emph{semi-direct product}:
\begin{equation}
G_{\Ho} = V_4 \rtimes_\varphi V_4
\label{eq:semidirect}
\end{equation}
where the first factor $V_4$ (the Klein four-group) acts on first-order relations (right--duty--liberty--no-right), the second factor $V_4$ acts on second-order relations (power--liability--immunity--disability), and the homomorphism $\varphi: V_4 \to \operatorname{Aut}(V_4)$ encodes how second-order operations act on first-order positions.  The dihedral extension $D_4 \rtimes_\varphi D_4$ is posited as the ambient group; it applies only if quarter-turn operations are independently demonstrated as normative operations.
\end{theorem}

\begin{proof}[Proof sketch]
Each Hohfeldian square admits $V_4$ symmetry by the same argument as in \cite{anon2026a}: the four positions form two correlative pairs connected by jural opposition (negation) and correlative swap (agent-swap).  These two operations are commuting involutions, so the group they generate is the Klein four-group $V_4$ (abelian, order 4); the full dihedral $D_4$ would require independent quarter-turn operations.
...
The group $G_{\Ho} = V_4 \rtimes_\varphi V_4$ has order at most 16 (the same as the direct product; the posited $D_4 \rtimes_\varphi D_4$ has order at most 64), but its multiplication rule encodes the coupling:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Non-commutativity of normative frameworks (16,798 commutator measurements).}  The order in which normative frameworks are applied changes the outcome ($[U, D] \neq 0$).  These contraction operators act on evaluations, not on Hohfeldian positions, so this result does not bear on the deontic gauge group (whose measured form, $V_4$, is abelian); it is nonetheless directly relevant to law, where the order of applying legal tests (e.g., standing before merits, jurisdiction before substance) produces different analytical paths.
...
The framework's cross-lingual predictions have been examined in a preliminary, unpublished analysis by Thiele \cite{thiele2026}, who reported all nine normative dimensions as linearly decodable from LaBSE embeddings (mean $F_1 = 0.83$) and found cross-lingual transfer across six languages ($F_1$ 0.71--0.82), which is carried by the multilingual (LaBSE) encoder's language-agnostic representation.  Most significantly, the moral-judgement subspace and the language-identity subspace were found to be nearly orthogonal (shared variance $< 3\%$). This is an unpublished single-author probe study, not peer-reviewed independent verification.
...
Preliminary external analysis (6 languages) & Orthogonality of legal and linguistic structure & Reported \\
...
Hohfeldian $V_4$ gauge group & Legal symmetry structure & Derived \\
...
This architecture directly replicates the methodology validated in the parent framework, where nine moral-dimension probes achieved $F_1 = 0.74$--$0.91$ \cite{anon2026a}, reported at $F_1 = 0.74$--$0.91$ in a preliminary, unpublished analysis by Thiele \cite{thiele2026}.  The legal probes use the same architecture with legal dimension labels.
...
\begin{itemize}
    ...
    \item \textbf{$D_4$ gauge operations} (the library implements the full posited dihedral group so the hypothesis stays testable) extend to operations on the full Hohfeldian octad.
