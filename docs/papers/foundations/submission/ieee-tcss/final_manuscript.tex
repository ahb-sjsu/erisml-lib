...
Three variances of the diagonal covariance are fitted on nine bargaining and public-goods targets by an exhaustive search over active dimensions.
...
Large replications show that many prospect-theory patterns are robust across countries while also exhibiting meaningful heterogeneity~\cite{ruggeri2020}, consistent with broader evidence of cross-cultural variation in economic behavior~\cite{henrich2010}.
...
$d_4$ & Autonomy & Freedom of choice, coercion aversion, agency & Development and capability approaches~\cite{sen1999} & Transferable \\
...
Third, the selected covariance is applied to the lottery and historical-replication targets without refitting the active variances, with the temperature constants set as described in Section~\ref{sec:temperature}.
...
\caption{Reference Points and Alternatives as Implemented. Shares $x$ and $f$ Are Fractions of the Stake or Endowment}
\label{tab:encodings}
\centering
\footnotesize
...
The labels P1, P3, and so on are Ruggeri et al.'s item numbers rather than the problem numbers of the original paper.
...
Section~\ref{sec:heldout} states the evidential consequence.
...
Section~\ref{sec:heldout} records what each benchmark target was used for and which result is independent of every fitting and selection step.
...
On the Fraser--Nettle game targets ($N=212$) the MAE, computed from unrounded observed values, is 0.17\% on the two ultimatum-game targets and 2.29\% on the five public-goods rounds.
...
On the Ruggeri items two of six predictions, P1 and P11, fall inside the country-pooled interval, and 58\% of the 114 country-by-item cells are covered.
Four items, P3, P7, P16, and P17, fall outside their Ruggeri interval despite passing the $\pm10$~pp tolerance.
At $N=4{,}098$ each interval extends roughly $\pm1$ to $\pm1.5$~pp around the observed frequency, so the 2.6~pp error on P3 and the 4.8~pp error on P7 fall outside it, as do the larger errors on P16 and P17.
...
\end{tabular}
\begin{flushleft}
\footnotesize{Error is predicted minus observed in percentage points. The overall MAE is the unweighted mean of the absolute errors over all sixteen targets. The Ruggeri-subset row reports the MAE on the six lottery items only, where $N=4{,}098$ per item gives the tightest comparison. Four lottery items, P3, P7, P16, and P17, pass the tolerance but fall outside the Ruggeri 95\% interval (Section~\ref{sec:statistical-eval}). The role column records which targets were used to fit the variances (calibration), to set the temperature constants, or to set the G\"uth reference coordinate. All sixteen targets were used to rank candidate active sets, so no target is fully out of sample. $^{\dagger}$P11 is encoded identically to P3. Reference implementation~\cite{eriseconsw}.}
\end{flushleft}
\end{table*}
...
Given the sensitivity of the model to epistemic displacements, a two-hundredths shift of one reference coordinate moves the predicted offer by thirteen points, from 48\% to 35\%, against an observed difference of eleven points.
...
Table~\ref{tab:sensitivity} presents the dimension-by-dimension sensitivity ranking and Fig.~\ref{fig:sensitivity} plots the same ablation as a profile.
The most important dimension is epistemic status ($d_9$).
Removing it causes the largest increase in MAE, from 2.70\% to the point of model failure, consistent with its role as the highest-sensitivity dimension.
Removing social impact ($d_6$) or virtue and identity ($d_7$) also degrades performance substantially.
The monetary dimension ($d_1$) has exactly zero sensitivity, and removing it or any other inactive dimension does not change the MAE at all.
...
The nine game predictions are exactly invariant to such a rescaling, because the minimizer of Eq.~\eqref{eq:argmin} does not depend on the overall scale of the cost, so only the six lottery frequencies move, through the temperature of Eq.~\eqref{eq:temperature}.
The model is therefore sensitive to the relative magnitudes of $\sigma_6^2$, $\sigma_7^2$, and $\sigma_9^2$, not to their absolute scale.
...
They were not refitted with the active variances, and they are not treated as externally preregistered facts.
...
\midrule
Geometric (this paper) & 13 named quantities (3 fitted variances) & 9/9 pass & 6/6 pass & 2.70\% (3.95\% lottery) & Yes, plus 1/1 historical \\
...
The geometric parameter entry counts the thirteen named quantities of Table~\ref{tab:param_accounting}, of which three are the fitted variances, before the coordinate constants of Table~\ref{tab:encodings}.
...
\subsubsection{Per-target error diagnostic}
...
It does mean that the parsimony claim should be read as ``few fitted variances conditional on an explicit encoding architecture,'' not as ``only three choices were made.''
...
\begin{enumerate}
    \item \textbf{Agent modeling}. Computational agents in social simulations can be endowed with a Mahalanobis cost function on nine dimensions as an alternative to scalar utility maximizers. Whether the resulting behavior is more realistic is not tested here.
...
\caption{Minimal agent-based illustration using the geometric metric as a behavioral primitive. The geometric agents spread offers almost uniformly from zero to the equal split, the expected-value agents offer near zero, and the inequality-averse population peaks near a 35 percent offer. The density axis is logarithmic so that the expected-value spike at zero and the two spread distributions are visible on one plot. The figure is illustrative and is not a new validation experiment.}
\label{fig:abm}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Narrow target battery}. The lottery set contains six classic targets, the historical replication target is a single 1982 ultimatum result, and the high-stakes boundary test uses one dataset from northeast India. A country-level analysis of the 19-country replication of Ruggeri et al.~\cite{ruggeri2020}, whose pooled frequencies are used here, additional high-stakes bargaining cohorts (Slonim and Roth~\cite{slonim1998}, Cameron~\cite{cameron1999}), non-WEIRD samples, trust games, and market-exchange datasets are necessary before broad claims can be made.
    ...
    \item \textbf{Tolerance-based evaluation}. The 16/16 pass rate is an engineering diagnostic. The MAE on the high-$N$ Ruggeri lottery subset is 3.95\%, against 2.70\% averaged over the full benchmark, and four of the six lottery predictions (P3, P7, P16, P17) fall outside the Ruggeri 95\% interval despite passing the $\pm10$~pp tolerance. Future work should adopt common train, validation, and test splits with individual-level likelihoods throughout.
