...
% OWNER: confirm the received date against ScholarOne before upload.
...
Three pre-specified tests reject stake-scaling invariance.
The monetary dimension is therefore inactive in the low-stakes tasks used for calibration and becomes active at consequential stakes, as the model predicts.
The paper specifies every encoding rule, search step, and fitted or coded constant of the reference implementation, states which held-out targets are untouched by any fitting, reports uncertainty where individual-level data are recoverable, and presents the contribution as a falsifiable cross-domain modeling architecture rather than a unification of game theory and prospect theory.
\end{abstract}
...
Two constants of the choice rule and one reference coordinate of the historical target were themselves set from held-out targets, and we say precisely which targets remain untouched by any fitting.
...
The model therefore behaves the way a falsifiable theory should.
The monetary dimension is inactive at the low stakes on which the metric was calibrated and becomes active at stakes that exceed a year of income.
...
It reports a cross-domain benchmark in which a covariance fitted on game-theoretic targets is evaluated on prospect-theory and historical-replication targets without refitting the active variances.
...
It reports a direct empirical falsification of universal money-zero on individual-level high-stakes data, which locates the boundary of the calibrated metric.
...
The two constants $T_{\text{base}}$ and $T_\alpha$ were set by requiring the model to reproduce the observed frequencies of two of those lotteries, the Allais problem P1 and the strong-certainty problem P3, given the fitted variances.
Those two targets are therefore not held out with respect to the temperature, and Section~\ref{sec:heldout} lists the targets that are.
...
Third, the selected covariance and temperature are applied to the held-out lottery and historical-replication targets without refitting the active variances.
...
We therefore rely on transparency and on the held-out structure described in Section~\ref{sec:heldout} rather than on a single degrees-of-freedom ratio as proof against overfitting.
%==============================================================================
...
\toprule
& \multicolumn{2}{c}{\textbf{Ultimatum proposer (offer $x$)}} & \multicolumn{2}{c}{\textbf{Responder (offer $x$)}} & \multicolumn{2}{c}{\textbf{Public goods (round $r$, share $f$)}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
\textbf{Dim.} & Ref.\ $\mathbf{r}$ & Alt.\ $\mathbf{s}(x)$ & Ref.\ $\boldsymbol{\rho}$ & Accept $\mathbf{s}(x)$ / Reject & Ref.\ $\mathbf{r}(r)$ & Alt.\ $\mathbf{s}(f)$ \\
\midrule
...
$d_6$ & $0$ & $-0.2+0.6\,m(x)-0.3P_{\text{rej}}(x)$ & $0$ & $0$ / $0$ & $-0.02(r-1)$ & $-0.4+0.8f$ \\
$d_7$ & $0.5$ & $0.3+0.5\,m(x)-0.2P_{\text{rej}}(x)$ & $0.6$ & $0.3+0.3\,m(x)$ / $0.7$ & $0.5$ & $0.2+0.6f$ \\
$d_8$ & $0.5$ & $0.5$ & $0.5$ & $0.5$ / $0.5$ & $0.5$ & $0.5$ \\
$d_9$ & $0.8$ (G\"uth $0.78$) & $0.5+0.3\,q(x)$ & $0.5$ & $0.5$ / $0.5$ & $0.5-0.03(r-1)$ & $0.5$ \\
\midrule
...
\midrule
$d_1$ & $1$ & $1-x$ & $0$ & \multicolumn{3}{l}{$\mathrm{EV}(L)/1000$, both alternatives then divided by the larger $|d_1|$} \\
...
The pool is multiplied by $m=2$ and divided equally.
...
In round $r$ the reference social-impact coordinate is $-0.02(r-1)$ and the reference epistemic coordinate is $0.5-0.03(r-1)$.
Under the selected metric the displacements are $\Delta a_6 = 0.8f-0.4+0.02(r-1)$, $\Delta a_7 = 0.6f-0.3$, and $\Delta a_9 = 0.03(r-1)$.
...
Lottery and historical-replication targets are excluded from $\mathcal{T}_{\text{cal}}$.
...
Because many candidates tie on the calibration objective, the reference implementation scores every candidate, at its calibrated variances, by the weighted MAE over all sixteen targets and ranks candidates by that score.
...
The ranking step uses the held-out targets to choose among active sets.
The held-out claim of this paper therefore applies to the variance values, which were fitted on game targets only, and not to the choice of active set.
...
The appropriate hedge is performance on targets untouched by any fitting, which Section~\ref{sec:heldout} lists, and the Andersen boundary test of Section~\ref{sec:andersen}, which is independent of every fitting step.
...
Lottery targets, which are held out from the variance fit, must be matched within $\pm10$~pp.
...
Three fitting steps touch the targets.
...
In addition, the ranking of candidate active sets used all sixteen targets.
Conditional on the selected active set, the targets whose values were not used in any fitting step are P7, P16, and P17, together with P11, whose encoding coincides with that of P3.
The cross-domain claim of the paper rests on those four lottery targets and on the Andersen test of Section~\ref{sec:andersen}.
Every other target is either a calibration target or was used to set one of the quantities listed above.
...
The 3.95\% figure on the Ruggeri subset is the most informative single MAE for held-out predictive accuracy because Ruggeri's large $N$ produces the tightest comparison, and we treat it as the conservative target metric.
...
12 & PT P7 Reflection             & Lottery, held out  & 79.4\%  & 84.2\%  & $+$4.8  & $\pm$10~pp & \checkmark \\
13 & PT P11 Isolation             & Lottery, held out$^{\dagger}$  & 16.1\%  & 15.4\%  & $-$0.7  & $\pm$10~pp & \checkmark \\
14 & PT P16 Small-prob.\ gain     & Lottery, held out  & 57.4\%  & 50.7\%  & $-$6.7  & $\pm$10~pp & \checkmark \\
15 & PT P17 Small-prob.\ loss     & Lottery, held out  & 42.8\%  & 50.6\%  & $+$7.8  & $\pm$10~pp & \checkmark \\
...
\end{tabular}
\begin{flushleft}
\footnotesize{Error is predicted minus observed in percentage points. The overall MAE is the unweighted mean of the absolute errors over all sixteen targets. The Ruggeri-subset row reports the MAE on the six lottery items only, where $N=4{,}098$ per item gives the tightest comparison. Two lottery items, P16 and P17, pass the tolerance but fall outside the Ruggeri 95\% interval (Section~\ref{sec:statistical-eval}). The role column records which targets were used to fit the variances (calibration), to set the temperature constants, or to set the G\"uth reference coordinate. $^{\dagger}$P11 is encoded identically to P3. Reference implementation~\cite{eriseconsw}.}
\end{flushleft}
\end{table*}
...
The social and identity displacements grow with the offer beyond about 20\%, while the epistemic penalty of rejection risk, weighted eighty times more heavily, shrinks with the offer.
...
P7, P16, and P17 were not used in any fitting step, and P11 shares its encoding with P3.
...
We therefore simulate encoding perturbation by drawing per-dimension scaling factors uniformly from $[1-\epsilon, 1+\epsilon]$ and adjusting~$\sigma_k^2$ accordingly.
...
\midrule
Geometric (this paper) & 3 & 9/9 pass & 6/6 pass & 2.70\% & Yes, plus 1/1 historical \\
...
CPT is evaluated on the six lottery problems and provides no mechanism for game predictions.
...
The EV maximizer is evaluated on the fifteen game and lottery targets.
MAE for CPT and Fehr--Schmidt is computed within their respective domains.}
\end{flushleft}
\end{table*}

The geometric model has lower benchmark MAE and a higher pass count than these baselines on this target set.
The comparison is suggestive rather than definitive, because the baselines and the geometric model are not estimated under a common individual-level likelihood protocol.
The point of the comparison is cross-domain applicability under a shared metric, not a final ranking of behavioral models.
...
\begin{itemize}
    ...
    We apply them to the same six lottery problems, computing $V(L) = \sum_i w(p_i) v(x_i)$ with the standard CPT value function and probability-weighting function.
...
As a pass-rate ceiling test we also optimized CPT's five parameters by Nelder--Mead from 500 random initializations, minimizing MAE on the six lottery targets with unconstrained search (initial ranges $\alpha, \beta \in [0.3, 2.0]$, $\lambda \in [1.0, 4.0]$, $\gamma, \delta \in [0.01, 1.0]$, with the search allowed to leave these ranges).
The best parameters found ($\alpha = 0.61$, $\beta = 1.50$, $\lambda = 2.59$, $\gamma = 0.12$, $\delta = 0.05$) achieve 4/6 pass with MAE\,=\,11.2\% and fail the same two targets as canonical CPT, P3 ($+14.2$~pp) and P11 ($+38.1$~pp).
The higher MAE relative to canonical CPT (11.2\% against 5.7\%) reflects a tradeoff.
The optimizer distorts the value and weighting functions to reduce error on P3 and P11 and worsens the targets that canonical CPT already matches.
The pass rate stays at 4/6 under both parameterizations.
Table~\ref{tab:comparison} reports the canonical result rounded to 5.8\% (the more precise figure is 5.7\%).
Per-target CPT errors under both parameterizations are given in Appendix~\ref{app:cpt}.

CPT achieves 4/6 pass on the six lottery problems with five parameters and cannot be applied to games.
...
The active set $\{d_6, d_7, d_9\}$ was discovered using the same nine game targets against which calibration accuracy is reported.
We address this with two analyses.

\subsubsection{Leave-one-out diagnostic}
For each of the nine game targets we examine how the model's prediction for that target relates to its predictions for the remaining fifteen.
The worst single-target error is $4.3$~pp (public-goods round~1, tolerance $\pm5$~pp) and the mean absolute error across the nine game targets is $1.96$~pp, so no single target dominates the calibration MAE.
...
\subsubsection{Calibration and held-out MAE}
The nine game targets have MAE\,=\,1.96\%, and the seven lottery and historical targets have MAE\,=\,3.67\%.
The held-out MAE is higher, as expected, and all seven targets pass within their wider $\pm10$~pp tolerance.
The ratio of the two MAEs is $1.87$.
The four lottery targets untouched by any fitting step, P7, P11, P16, and P17, have MAE\,=\,5.0\%.
The variances were selected to minimize error on game targets, and the lottery targets played no role in fitting them.
That the three-dimensional metric fitted on games also organizes the lottery phenomena is the paper's central empirical result, and its evidential weight rests on the targets listed in Section~\ref{sec:heldout}.
...
Points cluster around the line of perfect prediction with no systematic bias.
...
\caption{Direct test of the stake-scaling-invariance corollary on the data of Andersen et al.~\cite{andersen2011} ($N=458$, Meghalaya, India). Bars show observed means with 95\% confidence intervals. The dashed line is the prediction under an inactive monetary dimension, anchored to the Rs~20 condition. Both panels reject invariance, with $p=4\times10^{-13}$ for offers and $p=10^{-3}$ for rejections.}
\label{fig:andersen}
\end{figure}

\subsubsection{Interpretation}
The model behaves the way a falsifiable theory should when challenged outside its calibration setting.
Its most counterintuitive prediction fails, in the direction the model itself anticipates.
The selected metric remains a parsimonious descriptive model of the low-stakes tasks on which it was calibrated.
It is not a universal claim about rational choice, and the universal version is empirically false.

A complementary reading is that the active set $\{d_6,d_7,d_9\}$ describes a setting in which the social and epistemic costs of an offer dominate the monetary cost.
At a stake above a year's income the monetary cost is no longer negligible relative to those social and epistemic signals.
A model refitted on such data would need a monetary coordinate that carries the absolute stake, for example in units of local income, together with a finite $\sigma_1^2$.
The model accommodates this through context-specific covariance estimation.
The monetary dimension is inactive in the calibrated setting and becomes active where money carries decision-relevant weight that the social and epistemic dimensions cannot absorb.
Mapping the boundary explicitly, meaning the stake level at which $\sigma_1^2$ becomes finite and how that depends on local income and anonymity, is a natural target for follow-up work using individual-level data from Andersen et al., Slonim and Roth~\cite{slonim1998}, Cameron~\cite{cameron1999}, and competitive double-auction settings~\cite{smith1962}.
...
The present test should be read as the first of several boundary-condition probes rather than as a complete map of where $d_1$ activates.

\subsection{A Cross-Domain Bridge, Not a Unification}

The central methodological contribution is cross-domain prediction.
A metric fitted on game-theory data, meaning strategic interaction with real money, organizes six classic prospect-theory phenomena, meaning hypothetical lottery choices, and a corollary of that metric is then tested on a high-stakes ultimatum dataset that played no role in any fitting step.
...
The present paper takes the first such step, from games to lotteries, and demonstrates one direction in which the metric must change, since high stakes activate $d_1$.

\subsubsection*{Toward a tractable equilibrium model}
The emphasis on metric decision spaces also bears on a longstanding open problem in game theory.
Nash equilibria exist in finite games~\cite{nash1950}, but computing one is PPAD-complete~\cite{daskalakis2009,chen2009}, and the intractability worsens in continuous-action and multi-agent settings.
No actual agent, human or artificial, can be performing the global optimization that standard equilibrium analysis implicitly attributes to it.
Bounded rationality is therefore not only a psychological observation but a computational necessity.
Tractable behavior requires heuristic truncation of an otherwise intractable search.
A decision space equipped with an interpretable cost metric is the kind of structured search space on which directed search with admissible heuristics, in the style of the A$^{*}$ algorithm~\cite{hart1968}, replaces global optimization with tractable local navigation.
Under this reading the interpretable dimensions are candidate heuristic coordinates on the equilibrium landscape, namely the directions along which an admissible truncation of the multi-attribute equilibrium computation can be carried out without sacrificing optimality on the truncated subspace.
Developing that computational interpretation is beyond the scope of the present empirical paper.
...
We report the search exposure and rely on the targets untouched by fitting, the ablations, and the independent Andersen probe rather than on significance language.
...
A particular encoding-and-metric system reproduces a small set of canonical aggregate frequencies from two literatures with three fitted variances, conditional on the encodings, and the model's most counterintuitive corollary fails in the direction the model predicts when probed outside its calibration setting.
...
A variance is small when small displacement on that coordinate produces large changes in observed choice frequencies, regardless of whether the coordinate would be considered morally weighty or psychologically primitive.
...
The geometric primitive produces a modal offer near the equal-split region, which illustrates how a named metric can generate macro-level offer distributions in a simulation.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/abm_worked_example.pdf}
\caption{Minimal agent-based illustration using the geometric metric as a behavioral primitive. The geometric agents produce a modal offer near the equal split, the expected-value agents offer near zero, and the inequality-averse population spreads between them. The figure is illustrative and is not a new validation experiment.}
\label{fig:abm}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Fitting steps beyond the variances}. The temperature constants were set from P1 and P3, the G\"uth reference coordinate was set from the G\"uth offer, and the ranking of active sets used all sixteen targets. The lottery targets untouched by any fitting are P7, P11, P16, and P17.
    ...
    \item \textbf{Money-zero boundary located, not characterized}. The Andersen test rejects universal stake-scaling invariance but does not map the boundary across cultures, institutional settings, or stake ranges. Whether the same recalibration mechanism explains analogous boundaries in dictator and public-goods games, or in cohorts outside northeast India, remains open.
...
A diagonal covariance fitted on game-theoretic aggregate targets matches a small set of lottery and historical-replication targets without refitting the active variances.
...
The model's most counterintuitive prediction, stake-scaling invariance, is tested on the high-stakes ultimatum data of Andersen et al.~\cite{andersen2011} from Meghalaya, India ($N=458$, a thousandfold stake range), and all three pre-specified tests reject invariance.
The model therefore behaves the way a falsifiable theory should.
A sharp corollary fails sharply outside the calibration setting, in the direction the theory anticipates.
...
Its broader validity depends on stronger tests still to come, namely individual-level joint estimation across the full benchmark, multi-country lottery replications under a common protocol, additional high-stakes bargaining cohorts to map the $d_1$ activation boundary, market-exchange data, non-diagonal covariance structures, and preregistered encoding rules with external raters.
...
Table~\ref{tab:cpt_detail} reports CPT predictions for each lottery target under both the canonical parameters from Tversky and Kahneman~\cite{tversky1992} and the best parameters found by unconstrained Nelder--Mead optimization from 500 random initializations.
The optimization is a pass-rate ceiling test.
Even with full freedom to refit, CPT does not exceed 4/6 pass on these six problems, and P3 and P11 fail under both parameterizations.

\begin{table}[!h]
\renewcommand{\arraystretch}{1.15}
\caption{CPT Per-Target Errors, Canonical and Optimized}
...
Optimized parameters are $\alpha=0.61$, $\beta=1.50$, $\lambda=2.59$, $\gamma=0.12$, $\delta=0.05$.
The pass criterion is $|\text{Error}| \leq 10$~pp.
Both parameterizations fail P3 and P11.}
\end{flushleft}
\end{table}
...
\emph{Econ. Inquiry}, vol.~37, no.~1, pp.~47--59, Jan. 1999.
...
% OWNER: replace IEEEbiographynophoto with \begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{bond_photo}}]{Andrew H. Bond} once a photo file is added, and confirm the biography text.
