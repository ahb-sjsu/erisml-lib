...
def analyze_results(results: list[dict]) -> dict:
    ...
    valid_results = [
        r
        for r in results
        if r.get("order_A_verdict") != "ERROR" and r.get("order_B_verdict") != "ERROR"
    ]
    ...
    ci_low, ci_high = proportion_confint(
        n_effects, n_total, alpha=0.05, method="wilson"
    )
    ...
    for null in [0.05, 0.10, 0.15, 0.20]:
        ...
        z = (
            (effect_rate - null) / (null * (1 - null) / n_total) ** 0.5
            if n_total > 0
            else 0
        )
        sigma = (
            stats.norm.ppf(1 - result.pvalue)
            if result.pvalue > 1e-15 and result.pvalue < 0.5
            else (8.0 if result.pvalue < 1e-15 else 0)
        )
    ...
    for v in by_verdict:
        by_verdict[v]["rate"] = (
            by_verdict[v]["effects"] / by_verdict[v]["total"]
            if by_verdict[v]["total"] > 0
            else 0
        )
...
def print_report(analysis: dict, results: list[dict]):
    ...
    print(
        f"Order Effects Detected: {analysis['n_effects']} ({analysis['effect_rate']:.1%})"
    )
    ...
    for null_name, test in analysis["statistical_tests"].items():
        ...
        print(
            f"  vs {null_pct:>4} null: p = {test['p_value']:.2e}, z = {test['z_score']:.2f}, ~{test['sigma']:.1f}σ"
        )
    ...
    print(
        f"  Contested:     {cvc['contested_effects']}/{cvc['contested_total']} = {cvc['contested_rate']:.1%}"
    )
    ...
    print(
        "moral judgment {'DOES' if sigma >= 3 else 'may'} exhibit quantum-like non-commutativity."
    )
...
def main():
    ...
    parser.add_argument(
        "--data", required=True, help="Path to CSV file with AITA posts"
    )
    parser.add_argument("--api-key", required=True, help="Anthropic API key")
    parser.add_argument(
        "--n-posts", type=int, default=50, help="Number of posts to test (default: 50)"
    )
    parser.add_argument(
        "--model", default="claude-sonnet-4-20250514", help="Model to use"
    )
    parser.add_argument(
        "--output", default="qnd_results.json", help="Output file for results"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stratified", action="store_true", help="Stratify sample by verdict"
    )
    ...
    if args.stratified:
        ...
        if remaining > 0:
            extra = df[df["verdict"] == "YTA"].sample(
                min(remaining, len(df[df["verdict"] == "YTA"]))
            )
    ...
    for i, post in enumerate(sample):
        ...
        result = run_single_test(
            client, post, delay_between_orders=args.delay, model=args.model
        )
        ...
        if (i + 1) % 10 == 0:
            ...
            print(
                f"         --- Progress: {effects}/{len(valid)} effects ({rate:.1%}) ---"
            )
