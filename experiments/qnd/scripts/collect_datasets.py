...
def prepare_aita_for_qnd(
    parquet_path: Path, output_path: Path, sample_size: int = 1000
):
    ...
    if len(df) > sample_size:
        ...
        if len(sampled) < sample_size:
            ...
            extra = remaining.sample(
                min(len(remaining), sample_size - len(sampled)), random_state=42
            )
    ...
    for _, row in df.iterrows():
        qnd_data.append(
            {
                "id": hashlib.md5(
                    str(row.get("title", "") + str(row.get("text", ""))).encode()
                ).hexdigest()[:12],
                ...
                "comment1": (
                    str(row.get("comment1", ""))[:1000]
                    if pd.notna(row.get("comment1"))
                    else ""
                ),
                "comment2": (
                    str(row.get("comment2", ""))[:1000]
                    if pd.notna(row.get("comment2"))
                    else ""
                ),
            }
        )
...
def prepare_scruples_for_qnd(
    scruples_dir: Path, output_path: Path, sample_size: int = 500
):
    ...
    templates = [
        {
            ...
            "contexts": [
                ...
                "borrowed money and didn't pay back",
                "made a rude comment",
                "excluded me from an event",
                "damaged my property",
            ],
        }
    ]
...
Examples:
    ...
    parser.add_argument(
        "--minimal", action="store_true", help="Download minimal test set"
    )
    parser.add_argument(
        "--aita", action="store_true", help="Download AITA datasets only"
    )
    parser.add_argument(
        "--ethics", action="store_true", help="Download ethics benchmarks only"
    )
    parser.add_argument(
        "--prepare", action="store_true", help="Also prepare data for QND format"
    )
    parser.add_argument(
        "--output-dir", type=str, default="./qnd_datasets", help="Output directory"
    )
    parser.add_argument(
        "--sample-size", type=int, default=1000, help="Sample size for QND preparation"
    )
    parser.add_argument("--dataset", type=str, help="Download specific dataset by ID")
    parser.add_argument(
        "--synthetic",
        action="store_true",
        help="Generate synthetic test data (no network required)",
    )
    parser.add_argument(
        "--synthetic-size", type=int, default=100, help="Number of synthetic samples"
    )
    ...
    if args.prepare:
        ...
        if aita_path.exists():
            prepare_aita_for_qnd(
                aita_path, output_dir / "qnd_aita_prepared.json", args.sample_size
            )
