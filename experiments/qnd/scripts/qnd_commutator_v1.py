...
def analyze_commutators(results: Dict, specs: List[Dict]) -> Dict:
    ...
    for target in AXIS_NAMES:
        ...
        baseline_se = math.sqrt(baseline * (1 - baseline) / baseline_n) if baseline_n > 1 else 1
        ...
        for context in AXIS_NAMES:
            ...
            context_se = math.sqrt(context_mean * (1 - context_mean) / context_n) if context_n > 1 else 1
...
def print_analysis(analysis: Dict, output_dir: Path):
    ...
    if analysis["significant"]:
        print(f"{'Target':<12} {'Context':<12} {'[C,T]':>8} {'t-stat':>8} {'P(T|T)':>8} {'P(T|C)':>8}")
        print("-" * 64)
        
        for eff in analysis["significant"][:20]:  # Top 20
            print(f"{eff['target']:<12} {eff['context']:<12} "
    ...
    for target, contexts in matrix.items():
        for context, data in contexts.items():
            if data.get("commutator") and abs(data["commutator"]) > 0.1:
                strong_pairs.append((target, context, data["commutator"], data.get("significant", False)))
    ...
    axis_sensitivity = {}
    for target, contexts in matrix.items():
        effects = [abs(d["commutator"]) for d in contexts.values() 
