...
TENSE_NAMES = {
    Tense.PAST: {"en": "past", "ja": "過去", "ar": "الماضي", "zh": "过去", "hi": "भूतकाल", "is": "þátíð"},
    Tense.PRESENT: {"en": "present", "ja": "現在", "ar": "الحاضر", "zh": "现在", "hi": "वर्तमान", "is": "nútíð"},
    Tense.FUTURE: {"en": "future", "ja": "未来", "ar": "المستقبل", "zh": "未来", "hi": "भविष्य", "is": "framtíð"},
}
...
Result: The trolley diverts and kills the 1 worker. The 5 are saved, but the worker dies because B did not pull.""",
            "axes": {
                "primary": ("Individual Causation", "Did {subject} act wrongly in their individual choice?"),
                "secondary": ("Cooperative Expectation", "Did {subject} fail their cooperative obligation?")
            }
        },
        Language.JAPANESE: {
            "title": "トロッコの対峙",
...
結果：トロッコは側線に逸れ、1人の作業員が死にました。5人は救われましたが、Bが引かなかったため作業員は死にました。""",
            "axes": {
                "primary": ("個人的因果関係", "{subject}は個人的な選択において間違った行動をしましたか？"),
                "secondary": ("協力の期待", "{subject}は協力義務を果たさなかったでしょうか？")
            }
        },
        Language.ARABIC: {
            "title": "مواجهة العربة",
...
النتيجة: انحرفت العربة وقتلت العامل. أُنقذ الخمسة لكن العامل مات لأن ب لم يسحب.""",
            "axes": {
                "primary": ("السببية الفردية", "هل تصرف {subject} بشكل خاطئ في اختياره الفردي؟"),
...
Result: Both get 5 years. If neither had confessed, both would serve only 1 year.""",
            "axes": {
                "primary": ("Loyalty", "Did {subject} act wrongly by betraying their partner's trust?"),
                "secondary": ("Rational Self-Interest", "Was {subject}'s choice to confess rationally justified?")
            }
        },
        Language.JAPANESE: {
            "title": "相互の自白",
...
結果：両者5年。両者黙秘なら1年だった。""",
            "axes": {
                "primary": ("忠誠心", "{subject}は相棒の信頼を裏切って間違った行動をしましたか？"),
                "secondary": ("合理的自己利益", "{subject}の自白は合理的に正当化されますか？")
            }
        },
        Language.ARABIC: {
...
النتيجة: كلاهما 5 سنوات. لو لم يعترفا لكانت سنة فقط.""",
            "axes": {
                ...
                "secondary": ("المصلحة الذاتية العقلانية", "هل كان اختيار {subject} بالاعتراف مبرراً عقلانياً؟")
            }
        },
    },
    "sacrifice_lottery": {
...
Doctor harvests C's organs. C dies. A and B survive.""",
            "axes": {
                "primary": ("Individual Authorization", "Did {subject} commit moral wrong by saying 'yes'?"),
                "secondary": ("Shared Culpability", "Does {subject} bear responsibility for C's death?")
            }
        },
        Language.JAPANESE: {
            "title": "犠牲の抽選",
...
医師はCの臓器を摘出。Cは死亡。AとBは生存。""",
            "axes": {
                "primary": ("個人の承認", "{subject}は「はい」と言って道徳的過ちを犯しましたか？"),
...
الطبيب استأصل أعضاء ج. ج مات. أ وب نجيا.""",
            "axes": {
                "primary": ("التفويض الفردي", "هل ارتكب {subject} خطأً أخلاقياً بقول 'نعم'؟"),
                "secondary": ("المسؤولية المشتركة", "هل يتحمل {subject} مسؤولية عن موت ج؟")
            }
        },
    },
}
...
class DimConfig:
    ...
    def parse(cls, s: str):
        parts = s.split(":")
        return cls(Language(parts[0]), Tense(parts[1]) if len(parts) > 1 else Tense.PAST)


def generate_batch(n_trials, scenarios, languages, tenses, cross_lingual, cross_temporal, cross_dimensional, model):
    requests, specs = [], []
    
    def add_request(scenario_key, lang, tense, trial, subject, axis, cross_type, lang_a, lang_b, tense_a, tense_b):
        ...
        requests.append({
            "custom_id": cid,
            "params": {"model": model, "max_tokens": 200, "messages": [{"role": "user", "content": prompt}]}
        })
        specs.append({
            "custom_id": cid, "scenario": scenario_key, "trial": trial, "subject": subject,
            "axis": axis, "lang_a": lang_a, "lang_b": lang_b, "tense_a": tense_a, "tense_b": tense_b,
                ...
                for trial in range(n_trials):
                    for subj in ["A", "B"]:
                        for axis in ["primary", "secondary"]:
                            add_request(sc, lang, tense, trial, subj, axis, "mono",
                                       lang.value, lang.value, tense.value, tense.value)
    ...
    for lang_a, lang_b in cross_lingual:
        for tense in tenses:
            for sc in scenarios:
                for trial in range(n_trials):
                    for axis in ["primary", "secondary"]:
                        add_request(sc, lang_a, tense, trial, "A", axis, "xlang",
                                   lang_a.value, lang_b.value, tense.value, tense.value)
                        add_request(sc, lang_b, tense, trial, "B", axis, "xlang",
                                   lang_a.value, lang_b.value, tense.value, tense.value)
    ...
    for tense_a, tense_b in cross_temporal:
        for lang in languages:
            for sc in scenarios:
                for trial in range(n_trials):
                    for axis in ["primary", "secondary"]:
                        add_request(sc, lang, tense_a, trial, "A", axis, "xtemp",
                                   lang.value, lang.value, tense_a.value, tense_b.value)
                        add_request(sc, lang, tense_b, trial, "B", axis, "xtemp",
                                   lang.value, lang.value, tense_a.value, tense_b.value)
    ...
    for cfg_a, cfg_b in cross_dimensional:
        for sc in scenarios:
            for trial in range(n_trials):
                for axis in ["primary", "secondary"]:
                    add_request(sc, cfg_a.language, cfg_a.tense, trial, "A", axis, "xdim",
                               cfg_a.language.value, cfg_b.language.value, cfg_a.tense.value, cfg_b.tense.value)
                    add_request(sc, cfg_b.language, cfg_b.tense, trial, "B", axis, "xdim",
                               cfg_a.language.value, cfg_b.language.value, cfg_a.tense.value, cfg_b.tense.value)
    
    prereg = hashlib.sha256(json.dumps({"n": n_trials, "sc": scenarios, "req": len(requests)}, sort_keys=True).encode()).hexdigest()[:16]
...
def parse_verdict(text):
    ...
    try:
        ...
        if "NOT" in v: return 1, None
        elif "GUILTY" in v: return -1, None
    except: pass
    if re.search(r'\bNOT[_\s]?GUILTY\b', text, re.I): return 1, None
    elif re.search(r'\bGUILTY\b', text, re.I): return -1, None
...
def compute_chsh(results, specs):
    ...
    for cid, data in results.items():
        ...
        key = (spec.get("scenario"), spec.get("lang_a"), spec.get("lang_b"),
               spec.get("tense_a"), spec.get("tense_b"), spec.get("cross_type"))
    ...
    for key, trials in configs.items():
        ...
        corrs = {(a, b): [] for a in ["primary", "secondary"] for b in ["primary", "secondary"]}
        
        for td in trials.values():
            for aa in ["primary", "secondary"]:
                for ab in ["primary", "secondary"]:
                    va, vb = td.get(("A", aa)), td.get(("B", ab))
                    if va and vb: corrs[(aa, ab)].append(va * vb)
        
        def E(c): return sum(c)/len(c) if c else 0
        def se(c): 
            if len(c) < 2: return 1.0
            m = E(c)
            return math.sqrt(sum((x-m)**2 for x in c) / (len(c)-1) / len(c))
        
        Epp, Eps, Esp, Ess = E(corrs[("primary","primary")]), E(corrs[("primary","secondary")]), E(corrs[("secondary","primary")]), E(corrs[("secondary","secondary")])
        S = Epp - Eps + Esp + Ess
        se_S = math.sqrt(se(corrs[("primary","primary")])**2 + se(corrs[("primary","secondary")])**2 + 
                        se(corrs[("secondary","primary")])**2 + se(corrs[("secondary","secondary")])**2)
        
        violation = abs(S) > 2.0
        sigma = (abs(S) - 2.0) / se_S if se_S > 0 and violation else 0
        
        chsh_results.append({
            "scenario": scenario, "lang_a": lang_a, "lang_b": lang_b,
            "tense_a": tense_a, "tense_b": tense_b, "cross_type": cross_type,
            "n_trials": len(trials), "S": S, "se_S": se_S, "violation": violation, "sigma": sigma,
            "E_pp": Epp, "E_ps": Eps, "E_sp": Esp, "E_ss": Ess
        })
...
def print_summary(results, errors, prereg, output_dir):
    ...
    print("S = E(a,b) - E(a,b') + E(a',b) + E(a',b') | Classical: |S|≤2 | Quantum: |S|≤2.83")
    print("-"*70)
    ...
    names = {"mono": "MONO", "xlang": "CROSS-LANG", "xtemp": "CROSS-TEMP", "xdim": "CROSS-DIM"}
    violations = []
    ...
    for ct in ["mono", "xlang", "xtemp", "xdim"]:
        ...
        for r in sorted(by_type[ct], key=lambda x: -x["sigma"]):
            ls = f"{r['lang_a']}" if r['lang_a']==r['lang_b'] else f"{r['lang_a']}-{r['lang_b']}"
            ts = f"{r['tense_a'][:3]}" if r['tense_a']==r['tense_b'] else f"{r['tense_a'][:3]}-{r['tense_b'][:3]}"
            v = f"★{r['sigma']:.1f}σ" if r["violation"] else ""
            print(f"  {r['scenario'][:12]:<12} {ls:<6} {ts:<8} S={r['S']:+.3f}±{r['se_S']:.3f} n={r['n_trials']:<3} {v}")
            if r["violation"]: violations.append(r)
    ...
    artifact = {"prereg": prereg, "timestamp": datetime.now().isoformat(), "results": results, "n_errors": len(errors)}
...
def main():
    ...
    p.add_argument("--scenarios", nargs="+", default=["trolley_standoff", "mutual_confession", "sacrifice_lottery"])
    ...
    cross_lingual = [(lang_map[a], lang_map[b]) for p in args.cross_lingual for a,b in [p.split("-")] if a in lang_map and b in lang_map]
    cross_temporal = [(tense_map[a], tense_map[b]) for p in args.cross_temporal for a,b in [p.split("-")] if a in tense_map and b in tense_map]
    cross_dimensional = []
    for p in args.cross_dimensional:
        parts = p.split("-")
        if len(parts) == 2:
            cross_dimensional.append((DimConfig.parse(parts[0]), DimConfig.parse(parts[1])))
    
    if args.mode == "submit":
        reqs, specs, prereg = generate_batch(args.n_trials, args.scenarios, languages, tenses, cross_lingual, cross_temporal, cross_dimensional, args.model)
        
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        with open(output_dir / f"specs_{ts}.json", 'w') as f: json.dump({"prereg": prereg, "specs": specs}, f)
        ...
        with open(output_dir / f"batch_{ts}.json", 'w') as f:
            json.dump({"batch_id": batch.id, "prereg": prereg, "n_requests": len(reqs)}, f)
    
    elif args.mode == "status":
        batch = client.messages.batches.retrieve(args.batch_id)
        print(f"Batch: {args.batch_id} | Status: {batch.processing_status} | Counts: {batch.request_counts}")
    
    elif args.mode == "results":
        specs_files = sorted(output_dir.glob("specs_*.json"))
        with open(specs_files[-1]) as f: data = json.load(f)
        ...
        for r in client.messages.batches.results(args.batch_id):
            if r.result.type == "succeeded":
                ...
                results[r.custom_id] = {"spec": specs_map.get(r.custom_id, {}), "verdict": v, "error": e}
