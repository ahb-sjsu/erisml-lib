...
def parse_custom_id(custom_id: str, debug: bool = False) -> Optional[Tuple[int, str, str]]:
    """
        ...
        if part.isdigit():
            ...
            if i + 1 < len(parts):
                ...
                if potential_axes in ["pp", "ps", "sp", "ss"]:
                    ...
                    if debug:
                        print(f"  Found: trial={trial_idx}, axes={potential_axes}, subject={subject}")
    ...
    for i, part in enumerate(parts):
        if part.lower() in ["pp", "ps", "sp", "ss"]:
            ...
            if debug:
                print(f"  Fallback found: trial={trial_idx}, axes={axes}, subject={subject}")
...
def load_results(results_dir: Path, debug: bool = False) -> Tuple[Dict, Dict, str]:
    ...
    # Find specs file
    specs_files = list(results_dir.glob("*_specs.json")) + list(results_dir.glob("*specs*.json"))
    if not specs_files:
        ...
        if isinstance(specs_data, list):
            specs_by_id = {s.get("custom_id", str(i)): s for i, s in enumerate(specs_data)}
        else:
        ...
        if debug and specs_by_id:
            ...
            print(f"Sample spec: {json.dumps(specs_by_id[sample_key], indent=2, default=str)[:500]}")
        ...
        try:
            ...
            if isinstance(results, dict) and len(results) > 0:
                ...
                if debug:
                    ...
                    print(f"Sample result: {json.dumps(results[sample_key], indent=2, default=str)[:500]}")
...
def compare_models(results_by_model: Dict[str, List[CHSHResult]]):
    ...
    for scenario in sorted(scenarios):
        ...
        for model, results in sorted(results_by_model.items()):
            scenario_results = [r for r in results if r.scenario == scenario and not r.is_crosslingual]
            if scenario_results:
                # Average across languages for this model/scenario
                avg_S = sum(r.S for r in scenario_results) / len(scenario_results)
                avg_se = math.sqrt(sum(r.std_error**2 for r in scenario_results)) / len(scenario_results)
                ...
                print(f"{model:<15} {avg_S:>+8.3f} {avg_se:>8.3f} {max_sig:>6.1f} {violations:>10}")
        
        if len(s_values) >= 2:
            cv = (max(s_values) - min(s_values)) / (sum(s_values) / len(s_values)) if sum(s_values) != 0 else 0
...
def main():
    ...
    parser.add_argument("--compare", nargs="+", help="Compare multiple result directories")
