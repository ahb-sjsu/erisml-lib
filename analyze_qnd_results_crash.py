...
def get_lang_name(lang_str: str) -> str:
    ...
    if "JAPANESE" in lang_str:
        return "日本語"
    if "MANDARIN" in lang_str or "CHINESE" in lang_str:
        return "中文"
    if "SPANISH" in lang_str:
        return "Español"
    if "ARABIC" in lang_str:
        return "العربية"
...
def print_report(results: List[CHSHResult]):
    ...
    if results:
        ...
        max_sig = (
            max(r.significance for r in results)
        ...
        if cross_violations:
            ...
            print(
                "Could be linguistic artifact - cross-lingual test needed for confirmation."
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--results-dir", required=True, help="Directory with batch results"
    )
