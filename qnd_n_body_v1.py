    ...
    if topology == Topology.ALL:
        ...
        for i, name in enumerate(agent_names[:-1]):
            ...
            if i % 2 == 0:
                ...
                actions += f"- Person {name} keeps extra before passing to {next_name}\n"
...
- If all {n_agents} confess: Everyone gets {n_agents} years"""
    ...
    if n_conf == 0:
        ...
        outcome = f"Result: {confessors[0]} goes free. Others get {n_agents + 4} years each."
...
- Others can object but chief has final authority."""
    ...
    # Outcomes
    survivors = treated[:doctors] + [t for t in treated[doctors:] if random.random() > 0.5]
...
def compute_mermin_correlator(verdicts: Dict[Tuple[str, str], int], n_agents: int) -> float:
    """
...
def compute_chsh_extended(verdicts_by_trial: Dict[int, Dict[Tuple[str, str], int]], n_agents: int) -> Dict:
    """
...
class ExperimentConfig:
    ...
    def __post_init__(self):
        ...
        if len(self.languages) < self.n_agents:
            ...
            self.languages = [self.languages[i % len(self.languages)] for i in range(self.n_agents)]


def generate_requests(config: ExperimentConfig, seed: int = None) -> Tuple[List[Dict], List[Dict]]:
    ...
    for trial in range(config.n_trials):
        ...
        for i, agent in enumerate(agents):
            ...
            for axis in config.axes:
                ...
                lang_str = "-".join(l.value for l in config.languages[:config.n_agents])
                ...
                specs.append({
                    ...
                    "axis": axis.value,
                    "language": lang.value,
                    "languages": [l.value for l in config.languages[:config.n_agents]],
...
def generate_sweep(n_trials: int, model: str) -> Tuple[List[Dict], List[Dict], List[ExperimentConfig]]:
    ...
    all_languages = [Language.ENGLISH, Language.JAPANESE, Language.ARABIC, 
                    Language.CHINESE, Language.HINDI, Language.ICELANDIC]
...
def print_analysis(analysis: List[Dict], parse_errors: List, output_dir: Path):
    ...
    print("=" * 80)
    print("M = Mermin correlator | Classical: |M| ≤ bound | Quantum: |M| ≤ higher bound")
    print("=" * 80)
    ...
    for n in sorted(by_n.keys()):
        ...
        print(f"\n{'─' * 80}")
        print(f" N={n} AGENTS | Classical ≤ {bounds['classical']:.1f} | Quantum ≤ {bounds['quantum']:.2f}")
        print(f"{'─' * 80}")
        ...
        for r in results:
            lang_str = "-".join(r["languages"][:3]) + ("..." if len(r["languages"]) > 3 else "")
    ...
    print(f"Violations detected: {n_viol} ({100*n_viol/total:.1f}%)" if total > 0 else "")
    ...
    if violations:
        ...
        for n in sorted(by_n.keys()):
            ...
            if n_viols:
                ...
                print(f"  N={n}: {best['template']} {best['topology']} → {best['sigma']:.2f}σ")
...
def main():
    ...
    parser.add_argument("--n-agents", type=int, default=3, help="Number of agents (2-6)")
    parser.add_argument("--template", default="liferaft", 
                       choices=["liferaft", "trolley_network", "prisoners", "triage"])
    parser.add_argument("--topology", default="all", choices=["all", "chain", "star"])
    parser.add_argument("--axes", nargs=2, default=["deontological", "consequentialist"],
