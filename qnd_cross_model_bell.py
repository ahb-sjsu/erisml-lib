...
def calculate_chsh(measurements: List[Measurement], model_name: str) -> List[CHSHResult]:
...
def run_experiment(
    ...
    for model_key, model in models.items():
            ...
            for trial in range(n_trials):
                ...
                for subject in ["Person A", "Person B"]:
                    for axis in ["primary", "secondary"]:
                        m = run_measurement(model, scenario, subject, axis, trial, delay)
                        if m:
        ...
        for r in chsh_results:
            status = f"★ VIOLATION {r.significance:.1f}σ" if r.violation else "No violation"
        ...
        for model_key, results in sorted(results_by_model.items()):
            for r in results:
                if r.scenario == scenario:
                    ...
                    print(f"{r.model:<25} {r.S:>+8.3f} {r.std_error:>8.3f} {r.significance:>6.1f} {status:>10}")
...
def main():
    ...
    parser.add_argument("--output-dir", default="qnd_cross_model_results", help="Output directory")
    ...
    # Cost estimate
    n_calls = args.n_trials * len(SCENARIOS) * 4 * len([m for m in models if m != "rule"])
