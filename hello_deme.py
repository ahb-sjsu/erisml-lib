...
try:
    ...
    from erisml.ethics.facts import (
        EthicalFacts, Consequences, RightsAndDuties, JusticeAndFairness
    )
except ImportError:
...
def auto_fill(cls: Type, **overrides) -> Any:
    ...
    for field in dataclasses.fields(cls):
        if field.name in overrides:
            ...
            if field.type == bool: valid_args[field.name] = False
            elif field.type == float: valid_args[field.name] = 0.5
            elif field.type == int: valid_args[field.name] = 1
            elif field.type == str: valid_args[field.name] = "default"
            else: valid_args[field.name] = None
    return cls(**valid_args)

def run_governance_simulation(profile_id: str, options: List[str], json_output: bool) -> None:
    ...
    for opt in options:
        ...
        try:
            ...
            facts_obj = auto_fill(EthicalFacts,
                option_id=opt,
                consequences=cons,
...
def main():
    ...
    parser.add_argument("--options", nargs="+", default=["allocate_to_patient_A", "allocate_to_patient_B"])
