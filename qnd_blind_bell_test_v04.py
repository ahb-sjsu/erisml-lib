...
class BlindAPICaller:
    ...
    def make_blind_call(
        ...
        person_label = (
            "Person A" if subject_role == BlindRole.SUBJECT_ALPHA else "Person B"
        )
        ...
        user_prompt = create_measurement_prompt(
            scenario["content"], person_label, axis_info["prompt"], measurement_nonce
        )
...
def run_blinded_chsh_test(
    caller: BlindAPICaller, scenario_key: str, n_trials: int, verbose: bool = True
) -> BlindedCHSHResult:
    """
        ...
        for alpha_axis, beta_axis in settings:
            ...
            if alpha_result and beta_result:
                ...
                all_measurement_ids.extend(
                    [alpha_result.measurement_id, beta_result.measurement_id]
                )
...
def deblind_results(
    blinded_results: List[BlindedCHSHResult], scenario_keys: List[str]
) -> Dict[str, Any]:
    """
...
def print_blinded_report(
    results: List[BlindedCHSHResult], show_deblinding: bool = True
):
...
def print_deblinded_report(results: List[BlindedCHSHResult], scenario_keys: List[str]):
...
def main():
    ...
    parser.add_argument(
        "--api-keys-file", help="File with multiple API keys (one per line)"
    )
    parser.add_argument("--model", default="claude-sonnet-4-20250514")
    parser.add_argument(
        "--base-url", help="Alternative API base URL (for multi-datacenter)"
    )

    # Experiment parameters
    parser.add_argument(
        "--n-trials", type=int, default=30, help="Trials per measurement setting"
    )
    parser.add_argument(
        "--scenarios",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default="random",
        help="API key rotation strategy",
    )
    parser.add_argument(
        "--no-deblind",
        action="store_true",
    ...
    output_data = {
        ...
        "deblinding": (
            deblind_results(results, args.scenarios) if not args.no_deblind else None
        ),
