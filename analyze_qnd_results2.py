...
def analyze_order_effects(results: dict) -> dict:
    ...
    if analysis["total_posts"] > 0:
        analysis["detection_rate"] = (
            analysis["order_effects_detected"] / analysis["total_posts"]
        )
    else:
...
def analyze_interference(results: dict) -> dict:
    ...
    if analysis["interference_terms"]:
        ...
        analysis["variance"] = sum(
            (t - analysis["mean_interference"]) ** 2 for t in terms
        ) / len(terms)
...
def analyze_superposition(results: dict) -> dict:
    ...
    analysis = {
        ...
        "entangled_count": 0,
        "branch_counts": [],
        "by_ambiguity": defaultdict(
            lambda: {"superposed": 0, "entangled": 0, "total": 0}
        ),
    }
    ...
    # Calculate rates
    if analysis["total_posts"] > 0:
        analysis["superposition_rate"] = (
            analysis["superposed_count"] / analysis["total_posts"]
        )
        analysis["entanglement_rate"] = (
            analysis["entangled_count"] / analysis["total_posts"]
        )

    if analysis["branch_counts"]:
        analysis["mean_branches"] = sum(analysis["branch_counts"]) / len(
            analysis["branch_counts"]
        )
...
def generate_text_report(results: dict) -> str:
    ...
    report.append(
        f"Posts with Order Effects: {order_analysis['order_effects_detected']}/{order_analysis['total_posts']}"
    )
    report.append("")
    report.append("By Ambiguity Level:")
    for amb, data in order_analysis["by_ambiguity"].items():
        report.append(
            f"  {amb}: {data['detected']}/{data['total']} ({data['rate']*100:.1f}%)"
        )
    report.append("")
    ...
    if order_analysis["verdict_changes"]:
        ...
        for change in order_analysis["verdict_changes"][:5]:  # Show first 5
            report.append(
                f"  Post {change['post_id']}: {change['order_a']} → {change['order_b']}"
            )
    report.append("")
    ...
    if int_analysis.get("mean_interference") is not None:
        ...
        report.append(
            f"  Range: [{int_analysis['min_interference']:+.3f}, {int_analysis['max_interference']:+.3f}]"
        )
        report.append(f"  Variance: {int_analysis['variance']:.4f}")
    report.append("")

    has_interference = int_analysis["effect_counts"].get(
        "constructive", 0
    ...
    report.append("")
    report.append(
        f"Superposition Rate: {sup_analysis.get('superposition_rate', 0)*100:.1f}%"
    )
    report.append(
        f"Entanglement Rate: {sup_analysis.get('entanglement_rate', 0)*100:.1f}%"
    )
...
def create_visualizations(results: dict, output_dir: str = "."):
    ...
    if terms:
        ...
        ax.axvline(
            x=sum(terms) / len(terms),
            color="green",
            linestyle="-",
            label=f"Mean={sum(terms)/len(terms):.2f}",
        )
    ...
    for i, t in enumerate(thresholds):
        ax.hlines(
            y=t, xmin=i - 0.4, xmax=i + 0.4, color="gray", linestyle="--", linewidth=2
        )
    ...
    ax = axes[0]
    order_detected = [
        1 if p.get("order_effects", {}).get("order_effect_detected") else 0
        for p in posts
    ]
